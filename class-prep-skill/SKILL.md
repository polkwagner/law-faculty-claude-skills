...
description: >
  ...
  Use this skill whenever Polk asks to "prep a class", "prepare for class", "class prep",
...
You are preparing a complete set of class materials for a law school lecture in an
Intro to Intellectual Property course at the University of Pennsylvania Carey Law
School. The course uses the IPNTA casebook (2025 edition) and covers trade secrets,
patents, copyright, trademarks, and right of publicity. Classes run 80 minutes with
a 40-minute lecture, a 20-minute team problem exercise, and a 15-20 minute debrief.
...
1. **Identify the class session.** Determine the class number, topic, and date.

   - **CLI:** Ask the user for the course materials folder path. Read the syllabus
     to find the class session, topic, and reading assignment. Use Glob to find
     relevant files (e.g., `class_16.md`, slides, readings).
   - **Web:** Use `project_knowledge_search` to find the class markdown file
     (e.g., `class_16.md`) and the schedule entry. Extract the reading assignment.
     Search multiple times with different queries to ensure full coverage.

2. **Gather all inputs.** You need three things:
...
3. **Read everything thoroughly before analysis.** Read the full slide deck, the full
...
Target: **40-45 minutes** of lecture content (the class runs 80 minutes total,
with 20 minutes for the problem exercise and 15-20 for debrief/wrap-up).
...
5. **Complexity calibration**: Is the problem appropriate for ~20 minutes of
   focused group work? Flag if it's too simple (students will finish in 5
   minutes) or too complex (they won't get through it).
...
A .docx file following the polk-document skill formatting conventions:

- Read the polk-document skill first: `~/.claude/skills/polk-document/SKILL.md` (CLI)
  or `/mnt/skills/user/polk-document/SKILL.md` (web)
...
3. **Timing Plan**: A line-by-line breakdown allocating the 80-minute session:
   - Opening (~3 min)
   - Lecture sections with slide ranges and time estimates
   - Summary/takeaways (~2 min)
   - Class problem work (~20 min)
   - Problem discussion/debrief (~15-20 min)
   The lecture portion should target 38-42 minutes.
...
3. Produce and deliver the **Lecture Guide Document** as a .docx file. Use
   the polk-document skill for formatting conventions.
...
- Do not rewrite the slides or the class problem — this is a review and
  ...
  them as recommendations. (Use the `ip-problems` skill to revise problems
  and manually edit slides.)
