---
name: law-class-problems
description: >
  Revise and create class problems for a law school course. Use when asked to create,
...
You are revising or creating in-class problems for a law school course. Problems are
used for structured small-group adversarial exercises during class, where student teams
argue opposing sides.
...
2. **Identify the course.** Read the syllabus to determine the course name, subject
   matter, casebook, and doctrinal areas covered. If this is your first time working
   with this course, ask the user to confirm.

3. **Identify the class session.** Find the class number, topic, and assigned
   readings for the target session.

4. **Check for an existing class problem** for that session. If one exists,
   follow the **Revision Workflow**. If none exists, follow the **Creation Workflow**.

5. **Read the assigned reading materials** for that session.

6. **If a prior session's problem uses the same fact pattern** (problems
...
- Where the problem could touch on doctrines not yet covered, include an explicit
  scope note fencing them off (e.g., "Ignore [future topic] issues for now. Focus
  solely on [current topic].")
...
3. Design a scenario that naturally raises those issues:
   - Pick a realistic industry or setting where the legal issues arise organically
